...
echo. >> "%psFile%"
echo $CNAPY_VERSION = "1.2.4"  ^# Replace with the actual version if needed >> "%psFile%"
...
echo Copy-Item -Path "cnapy-1.2.4/condabin/mamba.bat" -Destination "cnapy-1.2.4/condabin/micromamba.bat" >> "%psFile%"
...
if exist "%psFile%" (
    ...
    echo To run CNApy, double-click on the newly created CNApy-1.2.4 desktop icon or,
    echo alternatively, double-click on the RUN_CNApy.bat file in the newly created cnapy-1.2.4 subfolder.
    echo To deinstall CNApy later, simply delete the newly created cnapy-1.2.4 subfolder.
    pause
) else (
