...
echo. >> "%psFile%"
echo $CNAPY_VERSION = "1.2.9"  ^# Replace with the actual version if needed >> "%psFile%"
...
echo Copy-Item -Path "cnapy-1.2.9/condabin/mamba.bat" -Destination "cnapy-1.2.9/condabin/micromamba.bat" >> "%psFile%"
...
if exist "%psFile%" (
    ...
    echo To run CNApy, double-click on the newly created CNApy-1.2.9 desktop icon or,
    echo alternatively, double-click on the RUN_CNApy.bat file in the newly created cnapy-1.2.9 subfolder.
    echo To deinstall CNApy later, simply delete the newly created cnapy-1.2.9 subfolder.
    pause
) else (
