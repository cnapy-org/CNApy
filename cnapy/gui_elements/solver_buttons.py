...
def get_solver_buttons(
    appdata: AppData, is_vertical: bool = True, for_straindesign: bool = True
) -> tuple[QVBoxLayout | QHBoxLayout, dict[str, QRadioButton | QButtonGroup]]:
    ...
    if module_exists("pyscipopt"):
        avail_solvers += [SCIP]
    if not for_straindesign and module_exists("highspy"):
        avail_solvers += ["HIGHS"]

    # Get solver button group
    solver_buttons_layout = QVBoxLayout() if is_vertical else QHBoxLayout()
    solver_buttons = {}
    ...
    if for_straindesign:
        # optlang_enumerator (for StrainDesign)
    ...
    if not for_straindesign:
        # HIGHS (for COBRA-k)
        # optlang_enumerator (for StrainDesign)
        solver_buttons["HIGHS"] = QRadioButton("HiGHS")
        solver_buttons["HIGHS"].setProperty("name", "HIGHS")
        solver_buttons["HIGHS"].setProperty("cobrak_name", "appsi_highs")
        solver_buttons["HIGHS"].setToolTip(
            "HIGHSpy solver, should have been automatically installed with CNApy :-)s"
        )
        solver_buttons_layout.addWidget(solver_buttons["HIGHS"])
        solver_buttons["group"].addButton(solver_buttons["HIGHS"])
